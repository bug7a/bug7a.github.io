...
- The content opens inside a full-screen `WebView` (iframe).
...
| `comp/` | Components used: `web-view.min.js` and `toast.min.js` (copies from comp-m4). |
...
- **Copies**: `basic/` and `comp/` are copies of the main folders (`../../basic/`, `../../comp-m4/`)
  and are not updated by themselves. `comp/` only holds `.min.js` files: copy `web-view.min.js` from
  `comp-m4/web-view.min.js`; `comp-m4/toast.js` has no `.min` twin, so build it with terser:
...
| `comp/` | Kullanilan bilesenler: `web-view.min.js` ve `toast.min.js` (comp-m4 kopyalari). |
...
- **Kopyalar**: `basic/` ve `comp/` ana klasorlerin (`../../basic/`, `../../comp-m4/`) kopyalaridir ve
  ...
  `comp-m4/web-view.min.js`'ten kopyala; `comp-m4/toast.js`'in `.min` ikizi olmadigi icin onu terser ile uret:
  `npx terser ../../comp-m4/toast.js --compress --mangle --comments /Bismillah/ -o comp/toast.min.js`
